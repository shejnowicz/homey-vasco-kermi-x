...
function buildModeCommand(raw, { mode, duration, nowMs }) {
  ...
  command.requestedLevel = requestedLevel;
...
function isModeConfirmed(state, request) {
  ...
  if (requestedLevel === undefined || state?.requestedMode !== requestedLevel) {
    return false;
  }
  ...
  switch (request?.duration?.type) {
    ...
    case 'minutes':
      return Number.isInteger(request.duration.minutes)

...
const MAX_FIREPLACE_MINUTES = 1_440;
...
function createManagedSession(state, minutes, nowMs) {
  const selectedMinutes = validatedFireplaceMinutes(minutes);
...
function isValidSession(value) {
  ...
  if (value.version !== VERSION || !isMode(value.priorMode)
    || !isValidFireplaceMinutes(value.selectedMinutes)
...
function validatedFireplaceMinutes(value) {
  if (!isValidFireplaceMinutes(value)) {
    throw new RangeError('Fireplace duration must be a whole number from 1 to 1440 minutes');
  }
  return value;
}

function isValidFireplaceMinutes(value) {
  return Number.isSafeInteger(value) && value >= 1 && value <= MAX_FIREPLACE_MINUTES;
}

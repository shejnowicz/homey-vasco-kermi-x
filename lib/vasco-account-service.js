...
const { isFireplaceCommand } = require('./vasco-command-builder');
...
class VascoAccountService {
  ...
  executeDeviceCommand(identity, build, confirm = null) {
    if (!isNonEmptyString(identity)) {
      return Promise.reject(new TypeError('A device identity is required'));
    }
    if (typeof build !== 'function' || (confirm !== null && typeof confirm !== 'function')) {
  ...
  _startRead() {
    ...
    read.then(configuration => {
      if (active.credentialVersion === this.credentialVersion) {
  ...
  async _executeDeviceCommand(identity, build, confirm) {
    const before = this.latestConfiguration ?? await this.readConfiguration();
    const device = findDevice(before, identity);
    const command = build(device.raw);
    let acknowledgedState = null;
    let acknowledgedWithoutState = false;
    ...
    await this._withSession(async session => {
      ...
      if (command.nextParameter === 'requestedLevel'
        ...
        // Controller support is experimental: a write acknowledgement alone
        // must not make Homey report an effective Controller state.
        if (command.nextValue !== 5) {
          const acknowledgedRaw = { ...command, level: command.nextValue };
          delete acknowledgedRaw.requestedLevel;
          delete acknowledgedRaw.nextParameter;
          delete acknowledgedRaw.nextValue;
          if (this.latestConfiguration === before) {
            this.latestConfiguration = replaceDeviceRaw(
              before,
              device.raw,
              acknowledgedRaw,
            );
      ...
      } else if (isFireplaceCommand(command)
        && typeof this.apiClient.writeDeviceParameter === 'function') {
        await this.apiClient.writeDeviceParameter({
          userToken: session.token,
          configuration: before,
        ...
        acknowledgedWithoutState = true;
      }
    });

    // Fireplace dataWritten confirms the write, not an observed device state.
    // Null is the successful acknowledgement sentinel until polling observes Vasco state.
    if (acknowledgedWithoutState) return null;
...
function replaceDeviceRaw(configuration, previousRaw, nextRaw) {
  if (!Array.isArray(configuration?.deviceProperties)) return configuration;
  const index = configuration.deviceProperties.indexOf(previousRaw);
  if (index < 0) return configuration;

  const deviceProperties = [...configuration.deviceProperties];
  deviceProperties[index] = nextRaw;
  return { ...configuration, deviceProperties };
}

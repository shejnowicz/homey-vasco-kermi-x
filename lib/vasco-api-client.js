...
const { VascoWebSocketClient } = require('./vasco-websocket-client');

const DEFAULT_BASE_URL = 'https://vasco.iqloud.eu/api/';
const DEFAULT_TIMEOUT_MS = 15_000;
const DEFAULT_WEBSOCKET_TIMEOUT_MS = 4_000;
...
class VascoApiClient {
  constructor({
    ...
    webSocketTimeoutMs = DEFAULT_WEBSOCKET_TIMEOUT_MS,
    clock = DEFAULT_CLOCK,
    webSocketClient = new VascoWebSocketClient({ timeoutMs: webSocketTimeoutMs, clock }),
  } = {}) {
  ...
  writeDeviceParameter(options) {
    return this.webSocketClient.writeParameter(options);
  }
